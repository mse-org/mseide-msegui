...
begin
 if length > 0 then begin
  lwo1:= not crc; //startvector
  po1:= data;
  po2:= data+length;
  while po1 < po2 do begin
   lwo1:= (lwo1 shr 8) xor crc32table[byte(lwo1) xor po1^];
   inc(po1)
  end;
  crc:= not lwo1;
 end;
...
function crc32(const data: pbyte; const length: integer): longword;
begin
 result:= 0;
 crc32(data,length,result);
end;
