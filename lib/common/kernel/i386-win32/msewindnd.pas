...
type
 ...
 tdataobject = class(tlinkedobject,idataobject)
  protected
   fdata: string;
   ftext: msestring;
   findex: integer;
 ...
 tsysdndhandler = class(teventthread,idroptarget,idropsource)
  protected
   fdragwinid: winidty;
 ...
 if (sysdndhandler = nil) then begin
  ...
  with sysdndhandler do begin
   case action of
    ...
    sdnda_begin: begin
     postevent(tsdndevent.create(sdndk_begin,0,act1,aintf));
     semwait;
    end;
    else begin
...
function tsysdndhandler.querycontinuedrag(fescapepressed: bool;
               grfkeystate: longint): hresult; stdcall;
begin
 result:= s_ok;
 if fescapepressed then begin
  ...
  if (grfkeystate and mk_lbutton = 0) then begin
   result:= dragdrop_s_drop;
  end;
 end;
end;

function tsysdndhandler.givefeedback(dweffect: longint): hresult; stdcall;
begin
 result:= s_ok;
end;
...
begin
 ...
 if result = s_ok then begin
  ...
  medium.hglobal:= globalalloc(0,getdatasize(po1));
...
begin
 ...
 for int1:= 0 to high(fformats) do begin
  fcformats[int1]:= registerclipboardformatw(
                             pwidechar(pmsechar(fformats[int1])));
 end;
end;

function tdataobject.queryinterface(
   {$ifdef fpc_has_constref}constref{$else}const{$endif} iid: tguid;
                                      out obj): hresult; stdcall;
begin
