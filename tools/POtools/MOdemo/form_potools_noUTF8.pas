...
var
  headerfo: theaderfo;
  forgoogle: Boolean;
  astro, astrt, acomp: msestring;  // utf8String;
...
begin
  ...
  begin
    ...
    strlang := {utf8}StringReplace(strlang, '@', '_', [rfReplaceAll]);
    ...
    begin
      ...
      file1.readln(str1);
      str2 := '';
      if (trim(str1) <> '') and ({UTF8}Copy(str1, 1, 1) <> '#') then
        if ({UTF8}Copy(str1, 1, 6) = 'msgstr') then
        begin
          str2 := {UTF8}Copy(str1, 7, length(str1));
          str2 := {utf8}StringReplace(str2, '\n', '', [rfReplaceAll]);
          str2 := {utf8}StringReplace(str2, '\', '', [rfReplaceAll]);
          str2 := {utf8}StringReplace(str2, '"', '', [rfReplaceAll]);
          if str2 <> '' then
          begin
    ...
    begin
      ...
      begin
        ...
        str2 := '';
        if (trim(str1) <> '') and ({UTF8}Copy(str1, 1, 1) <> '#') then
          if ({UTF8}Copy(str1, 1, 5) = 'msgid') then
          begin
            str2 := {UTF8}Copy(str1, 7, length(str1));
            str2 := {utf8}StringReplace(str2, '\n', '', [rfReplaceAll]);
            str2 := {utf8}StringReplace(str2, '\', '', [rfReplaceAll]);
